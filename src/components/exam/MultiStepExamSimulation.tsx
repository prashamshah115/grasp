...
import { Flag, ChevronLeft, ChevronRight, Sparkles } from 'lucide-react';
import { MultiStepQuestionCard } from '../shared/MultiStepQuestionCard';
...
interface MultiStepExamSimulationProps {
  examTitle: string;
...
export function MultiStepExamSimulation({
  examTitle,
  durationMinutes,
  questions,
  onComplete,
  onExit
}: MultiStepExamSimulationProps) {
  ...
  // Calculate if question is "answered" (at least one step has an answer)
  const isQuestionAnswered = (questionId: string) => {
    const questionAnswers = answers[questionId];
    return questionAnswers && Object.keys(questionAnswers).length > 0;
  };
  ...
  const aiContext = `Exam: ${examTitle}\nCurrent Question: ${currentQuestion.question}\nSteps: ${currentQuestion.steps.map(s => s.prompt).join(', ')}`;
        ...
        <div className="max-w-7xl mx-auto px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-4 py-2 bg-[#F5F3FF] rounded-[10px] border border-[#DDD6FE]">
                <Sparkles className="w-4 h-4 text-[#4F46E5]" />
                <span className="text-sm text-[#4F46E5] font-medium">AI Help Available</span>
              </div>
              <ExamTimer
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto px-8 py-12">
            ...
            {/* AI Hint */}
            <div className="mt-6 p-4 bg-[#F5F3FF] border border-[#DDD6FE] rounded-[12px]">
              <div className="flex items-start gap-3">
                <Sparkles className="w-5 h-5 text-[#4F46E5] flex-shrink-0 mt-0.5" />
                <div className="text-sm text-[#6B7280]">
                  <span className="font-medium text-[#374151]">Stuck?</span> The AI assistant can help explain concepts, break down steps, or give hints. Click the floating button to chat!
                </div>
              </div>
            </div>
