...
`;
function mapEdgeTypeToRelation(edgeType) {
  switch (edgeType) {
    case "prerequisite":
      return "prerequisite";
    case "strengthens":
      return "dependent";
    // Worked example depends on concept
    case "uses":
      return "dependent";
    // Example uses formula/concept
    case "similar_to":
      return "overlap";
    // Conceptual overlap
    default:
...
__name(mapEdgeTypeToRelation, "mapEdgeTypeToRelation");
...
__name(convertConfidence, "convertConfidence");
...
var generateKnowledgeGraph = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    for (const e of parsed.edges) {
      ...
      const llmConfidence = typeof e.confidence === "number" ? Math.min(3, Math.max(1, e.confidence)) : 2;
      edgeRows.push({
        course_id: courseId,
        ...
        to_object_id: toDbId,
        // Use actual UUID
        relation: mapEdgeTypeToRelation(edgeType),
        // Map to database relation type
        confidence: convertConfidence(llmConfidence),
...
//# sourceMappingURL=chunk-RB3SD2SP.mjs.map
