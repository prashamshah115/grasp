...
async function getProcessedDocumentIds(supabase, courseId) {
  const { data: objects, error } = await supabase.from("knowledge_objects").select("content").eq("course_id", courseId);
  if (error) {
...
__name(getProcessedDocumentIds, "getProcessedDocumentIds");
async function getDocumentsWithoutPages(supabase, documents) {
  const docIdsWithoutPages = /* @__PURE__ */ new Set();
  for (const doc of documents) {
    const { count } = await supabase.from("document_pages").select("*", { count: "exact", head: true }).eq("document_id", doc.id);
    if (count === 0) {
...
__name(getDocumentsWithoutPages, "getDocumentsWithoutPages");
...
var precomputeKnowledgeObjects = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    metadata.set("documentsAlreadyProcessed", processedDocIds.size).set("documentsToProcess", documentsToProcess.length).set("totalDocuments", documents.length);
    ...
    let formulaCount = 0;
    let exampleCount = 0;
    const skippedDocIds = /* @__PURE__ */ new Set();
    ...
    for (const doc of documentsToProcess) {
      ...
      try {
        ...
        const raw = await callLLM(messages, {
          temperature: 0.2,
          maxTokens: 8192
          // Increased for GPT-5 models - knowledge extraction can be large
        });
    ...
    const allSkippedDocIds = /* @__PURE__ */ new Set([...skippedDocIds, ...documentsWithoutPages]);
    ...
    if (remainingDocumentsToProcess.length > 0) {
      ...
      const { precomputeKnowledgeObjects: precomputeKnowledgeObjects2 } = await import("./grasp/trigger/tasks/index.mjs");
      await precomputeKnowledgeObjects2.trigger({
        courseId
      });
    ...
    const { generateKnowledgeGraph } = await import("./grasp/trigger/tasks/generate-knowledge-graph.mjs");
    const { precomputeFinalPacks } = await import("./grasp/trigger/tasks/precompute-final-packs.mjs");
    const { embedWebResults } = await import("./grasp/trigger/tasks/embed-web-results.mjs");
...
//# sourceMappingURL=chunk-PFXVVLWB.mjs.map
