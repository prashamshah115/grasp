...
// node_modules/@trigger.dev/core/dist/esm/v3/errors.js
var AbortTaskRunError = class extends Error {
  static {
    __name(this, "AbortTaskRunError");
  }
  constructor(message) {
    super(message);
...
async function generateJWT(options) {
  const { SignJWT } = await import("./esm-KUXZ7PEP.mjs");
...
var TaskContextAPI = class _TaskContextAPI {
  ...
  setGlobalTaskContext(taskContext2) {
    return registerGlobal(API_NAME3, taskContext2);
  }
  #getTaskContext() {
...
async function randomUUID() {
  const { randomUUID: randomUUID2 } = await import("./crypto.node-FV7VT4X2.mjs");
  return randomUUID2();
}
__name(randomUUID, "randomUUID");
async function digestSHA256(data) {
  const { subtle: subtle2 } = await import("./crypto.node-FV7VT4X2.mjs");
...
var APIClientManagerAPI = class _APIClientManagerAPI {
  ...
  runWithConfig(config, fn) {
    ...
    registerGlobal(API_NAME4, $config, true);
    return fn().finally(() => {
      registerGlobal(API_NAME4, originalConfig, true);
    });
  }
  setGlobalAPIClientConfiguration(config) {
    return registerGlobal(API_NAME4, config);
  }
  #getConfig() {
    return getGlobal(API_NAME4);
  }
...
async function loadSuperJSON() {
  const superjson = await import("./dist-2Q5S5PLY.mjs");
...
__name(carrierFromContext, "carrierFromContext");
...
@google-cloud/precise-date/build/src/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-F2S4DK4N.mjs.map
