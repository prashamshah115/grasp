...
// tasks/update-rag-cache.ts
init_esm();
var import_supabase_js = __toESM(require_main());
var updateRagCache = task({
  id: "update-rag-cache",
  queue: {
    concurrencyLimit: 2
  },
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    let documentsQuery = supabase.from("documents").select("id, topic_id, title").eq("course_id", courseId).eq("status", "ready");
    if (documentId) {
    ...
    const { data: concepts } = await supabase.from("concepts").select("id, title, topic_id").eq("course_id", courseId);
    const conceptsByTopic = /* @__PURE__ */ new Map();
    concepts?.forEach((c) => {
      if (c.topic_id) {
    ...
    const { data: formulas } = await supabase.from("formulas").select("id, topic_id").eq("course_id", courseId);
    const formulasByTopic = /* @__PURE__ */ new Map();
    formulas?.forEach((f) => {
      if (f.topic_id) {
    ...
    for (const doc of documents) {
      ...
      metadata.set("progress", docProgress).set("currentDocument", doc.title).set("documentsProcessed", processedDocs);
      const { data: pages } = await supabase.from("document_pages").select("id").eq("document_id", doc.id);
      if (!pages || pages.length === 0) {
        processedDocs++;
        continue;
      }
      const pageIds = pages.map((p) => p.id);
      const { data: chunks, error: chunksError } = await supabase.from("document_chunks").select("id, page_id, content, embedding, context_tags").in("page_id", pageIds);
      ...
      for (let i = 0; i < ragChunks.length; i += batchSize) {
        ...
        await supabase.from("rag_chunks").delete().in("chunk_id", chunkIds);
        const { error: insertError } = await supabase.from("rag_chunks").insert(batch);
        if (insertError) {
...
//# sourceMappingURL=chunk-PA65Y743.mjs.map
